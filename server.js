...
// ---- one-time reset: wipe ALL accounts, chats, contacts, presence ----
// Protected by RESET_KEY (set it as an env var). Visit:
//   https://your-app.onrender.com/admin/reset?key=YOUR_RESET_KEY
// Does nothing unless the key matches. Remove RESET_KEY (or this route) when done.
async function wipeAll() {
  // clear everything in the shared scope, and every personal scope
  await db.execute("DELETE FROM kv");
}
app.get("/admin/reset", async (req, res) => {
  const expected = process.env.RESET_KEY;
  if (!expected) {
    return res.status(403).send("Reset is disabled. Set a RESET_KEY environment variable to enable it.");
  }
  if (req.query.key !== expected) {
    return res.status(403).send("Wrong or missing reset key.");
  }
  try {
    await wipeAll();
    broadcast({ type: "change", key: "*" });
    res.send("✅ All accounts, chats, contacts, and presence have been wiped. Everyone can now sign up fresh. You can remove the RESET_KEY env var to disable this endpoint again.");
  } catch (e) {
    res.status(500).send("Reset failed: " + String(e));
  }
});
